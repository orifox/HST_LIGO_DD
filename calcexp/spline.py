    ...
    def calc_ypp(self):
        ...
        if self.use_low_slope:
            u[0] = (3.0/(x_vals[1]-x_vals[0])) * \
                ((y_vals[1]-y_vals[0])/(x_vals[1]-x_vals[0])-self.low_slope)
...
class LinInt(func.FuncOps):
	...
	# if out of range, return endpoint
        if x <= self.x_vals[0]:
            if self.use_low_slope:
                return self.low_slope*(x-self.x_vals[0])+self.y_vals[0]
            else:
                return self.y_vals[0]
        if x >= self.x_vals[-1]:
            if self.use_high_slope:
                return self.high_slope*(x-self.x_vals[-1])+self.y_vals[-1]
            else:
                return self.y_vals[-1]

...
#                    print lineinfo[38]
    ...
    def loadfile(self,filename,addflag=0,maxsplit=0,takelastheader=0, onlyparsecols=None,noheader=False,headerstring=None, loadstartline=None,headerindex=None):
        self.errorflag=0
...
#         self.parsetable(headerstring,lines,startindex=1,addflag=addflag,maxsplit=maxsplit)
    ...
    def save2cmpfile(self, filename, **args):
        if 'verbose' in args:
            print(('Saving ',filename))
        try:
            f = open(filename,'w')
        except (OSError, IOError) as e:
            message = '%s\nCould not open file %s for writing' %(e, filename)
            warnings.warn(message, RuntimeWarning)
            return
        header = self.cmpheader
        outlines = []
        if header is not None:
            try:
                keys  = list(header.keys())
            except AttributeError as e:
                message = '%s\nCould not get header keys for %s' %(e, filename)
                warnings.warn(message, RuntimeWarning)
                keys = []
            pos   = [header[key]['#'] for key in keys]
            lines = [header[key]['line'] for key in keys]
            pos, lines = list(zip(*sorted(zip(pos, lines))))
            outlines += lines
        if len(outlines) > 0:
            lstr = len(outlines[0]) - 3
            endline  = '%s%'+str(lstr)+'s'
            endline  = endline %('END','')
            outlines += [endline]
    ...
    def selectlckeys(self,keys=None,photcodes=None,dophottypes=None,onlyforced=0,onlynonforced=0,MJDmin=None,MJDmax=None,skipbadim=0,onlybadim=0,Nmaskmax=None,skipifpeakmasked=None):
        ...
        for lckey in keys:
            ...
            if photcodes != None:
                ...
                if lcpc == None:
                    continue
                lcpc = tools.hex2int(lcpc) & 0xffff
                foundflag = 0
                for pc in photcodes:
                    if pc == lcpc:
                        foundflag = 1
